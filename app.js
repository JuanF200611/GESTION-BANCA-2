...
let empleados = [];
let loterias = []; // Historial de resultados de lotería
...
window.switchLoginTab = function(tipo) {
    document.getElementById('btnTabEmpleado').classList.toggle('active', tipo === 'empleada');
    document.getElementById('btnTabAdmin').classList.toggle('active', tipo === 'admin');
    document.getElementById('loginEmpleadoForm').classList.toggle('hidden-pane', tipo !== 'empleada');
    document.getElementById('loginAdminForm').classList.toggle('hidden-pane', tipo !== 'admin');
};
...
function concluirLogin(panelCallback) {
    ...
    document.getElementById('userAvatarHeader').textContent = usuarioActual.avatar;
...
window.cerrarSesion = function() {
    ...
    document.getElementById('userAvatarHeader').textContent = '?';
    document.getElementById('pageTitle').textContent = 'Identificación...';
...
// Limpiar historial antiguo (más de 30 días) sin afectar montos actuales
window.limpiarHistorialCompletoBanca = async function(bancaKey) {
    ...
    // Limpiar préstamos antiguos ya retirados
    ...
    // Limpiar comisiones antiguas ya retiradas
    ...
    // Limpiar cuadres central antiguos ya retirados
    ...
    // Limpiar historial de cuadres antiguo
...
// Limpiar historial de un cliente específico
window.limpiarHistorialCliente = async function(clienteId, clienteNombre) {
    if(!confirm(`¿Limpiar historial de transacciones de "${clienteNombre}"? El saldo actual NO se afecta.`)) return;
    ...
    alert(`Historial de ${clienteNombre} limpiado exitosamente`);
    if(usuarioActual.rol === 'admin') cargarClientesAdmin(); else cargarClientesEmpleada();
};
...
window.abrirModalRetirar = function(tipo) {
    ...
    // Cargar lista de items no retirados
    let items = [];
...
window.cargarPanelEmpleada = function() {
    ...
    document.getElementById('contentArea').innerHTML = `
        ...
        <button class="mobile-btn btn-success-filled" onclick="abrirFlujoTransaccion()"><i class="fa-solid fa-square-plus"></i> Ticket / Cobro Rápido</button>
    `;
};
...
function renderListaClientesMovil(lista, esAdmin) {
    ...
    if(lista.length === 0) { html = '<p style="text-align:center; padding:20px; color:var(--text-muted);">Sin clientes en esta sección.</p>'; }

    lista.forEach(c => {
        let esMoroso = false;
        if (c.fechaActualizacion) {
            const horas = Math.abs(ahora - new Date(c.fechaActualizacion)) / 36e5;
            if (horas > 48) esMoroso = true;
        } else { esMoroso = true; }

        html += `
            <div class="mobile-card ${esMoroso ? 'moroso-card' : ''}">
                <div class="card-top">
                    <span class="card-title">${c.nombre}</span>
                    <span class="mobile-badge ${esMoroso ? 'badge-danger' : 'badge-success'}">${esMoroso ? 'Moroso (+48h)' : 'Al Día'}</span>
                </div>
                <div class="card-row-data">
                    <div class="data-cell"><label>Deuda</label><p>RD$ ${c.deuda || 0}</p></div>
                    <div class="data-cell"><label>Fondo</label><p>RD$ ${c.fondo || 0}</p></div>
                </div>
                <div class="card-actions">
                    <button class="mini-btn mini-btn-success" onclick="lanzarMovimientoDirecto('${c.id}', 'pago')">Cobrar</button>
                    <button class="mini-btn mini-btn-danger" onclick="lanzarMovimientoDirecto('${c.id}', 'deuda')">Ticket</button>
                    <button class="mini-btn mini-btn-info" onclick="lanzarRecordatorioPago('${c.id}')"><i class="fa-brands fa-whatsapp"></i> Mensaje</button>
                    <button class="mini-btn" onclick="abrirFichaDetalle('${c.id}', ${esAdmin})"><i class="fa-solid fa-eye"></i> Historial</button>
                    ${esAdmin ? `<button class="mini-btn mini-btn-danger" onclick="limpiarHistorialCliente('${c.id}', '${c.nombre}')"><i class="fa-solid fa-trash"></i> Limpiar Historial</button>` : ''}
                </div>
            </div>
        `;
    });
...
window.lanzarRecordatorioPago = function(id) {
    ...
    const msg = `Saludos *${c.nombre}*,\nLe informamos desde el *Consorcio Kiko*\nQue posee un balance pendiente de: *RD$ ${c.deuda || 0}*.\nPor favor acercarse a la banca para validar sus tickets pendientes. ¡Gracias!`;
...
window.lanzarMovimientoDirecto = function(id, tipo) {
    abrirFlujoTransaccion();
    document.getElementById('deudaCliente').value = id;
    document.getElementById('deudaTipo').value = tipo;
};
...
window.abrirFichaDetalle = function(id, esAdmin) {
    ...
    // Mostrar historial con filtro de fechas
    let historialHtml = `
    ...
    window.cargarHistorialCliente = function(clienteId) {
        ...
        let hHtml = '';
        filtradas.forEach(f => {
            hHtml += `<div style="padding:4px 0; border-bottom:1px solid #e2e8f0;">• ${f.fecha || ''} | ${f.monto > 0 ? 'Ticket':'Cobro'}: <b>RD$ ${Math.abs(f.monto)}</b> <span style="color:var(--text-muted); font-size:10px;">(${f.concepto || ''})</span></div>`;
        });
...
window.guardarCliente = async function() {
    ...
    const datos = {
        ...
        banca: document.getElementById('clienteBanca').value,
        deuda: parseFloat(document.getElementById('clienteDeuda').value) || 0,
        fondo: parseFloat(document.getElementById('clienteFondo').value) || 0,
        fechaActualizacion: new Date().toISOString()
    };
    if(!id) { await db.ref('clientes').push(datos); } else { await db.ref(`clientes/${id}`).update(datos); }
    cerrarModal('modalCliente');
    if(usuarioActual.rol === 'admin') cargarClientesAdmin(); else cargarClientesEmpleada();
};

// ========== CALCULADORA DE CUADRE (SOLO VISTA) ==========
window.cargarCalculadoraCuadre = function() {
    ...
    const dTotalClientes = clientes.filter(c => c.banca === bKey).reduce((s,c) => s + (c.deuda || 0), 0); // AUTOMÁTICO
    ...
    document.getElementById('contentArea').innerHTML = `
        ...
        <div class="summary-box" style="background:#fff;">
            ...
            <input type="number" id="cuadreDeudaTotalRestable" class="mobile-input" value="${dTotalClientes}" readonly style="background:#f0f0f0; font-weight:bold;" oninput="calcularMatrizCuadre()">
...
window.abrirFlujoTransaccion = function() {
    ...
    document.getElementById('deudaCliente').innerHTML = opts;
...
window.guardarDeuda = async function() {
    ...
    let nD = c.deuda || 0;
    if(tipo === 'deuda') nD += monto; else nD = Math.max(0, nD - monto);

    await db.ref('deudas').push({ clienteId: cid, monto: tipo==='deuda'?monto:-monto, concepto: conc, fecha: new Date().toLocaleDateString('es-DO'), timestamp: new Date().toISOString() });
    await db.ref(`clientes/${cid}`).update({ deuda: nD, fechaActualizacion: new Date().toISOString() });
    cerrarModal('modalDeuda');
    if(usuarioActual.rol === 'admin') cargarClientesAdmin(); else cargarPanelEmpleada();
};
...
document.addEventListener("DOMContentLoaded", () => {
    if(document.getElementById('currentDate')) document.getElementById('currentDate').textContent = new Date().toLocaleDateString('es-DO');
    cargarDatosIniciales();
});
